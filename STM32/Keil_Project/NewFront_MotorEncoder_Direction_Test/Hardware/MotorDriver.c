...
static uint8_t motor_pwm_started[MOTOR_ID_COUNT];
...
void MotorDriver_Init(void)
{
  ...
  motor_pwm_started[MOTOR_ID_A] = 1U;
  if (HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_2) != HAL_OK)
  {
    Error_Handler();
  }
  motor_pwm_started[MOTOR_ID_B] = 1U;
  if (HAL_TIM_PWM_Start(&htim5, TIM_CHANNEL_2) != HAL_OK)
  {
    Error_Handler();
  }
  motor_pwm_started[MOTOR_ID_C] = 1U;
  if (HAL_TIM_PWM_Start(&htim5, TIM_CHANNEL_3) != HAL_OK)
  {
    Error_Handler();
  }
  motor_pwm_started[MOTOR_ID_D] = 1U;
                                           ...
                                           MotorDirection_t electrical_direction)
{
  if ((Motor_IsValid(motor) == 0U) ||
      (motor_driver_enabled != 0U) ||
      ((electrical_direction != MOTOR_DIRECTION_FORWARD) &&
       (electrical_direction != MOTOR_DIRECTION_REVERSE)))
  {
    return 0U;
  }

  Motor_StopRaw(motor);
  Motor_SetDirectionPins(motor, electrical_direction);
  motor_direction[motor] = electrical_direction;
  return 1U;
}
...
uint8_t MotorDriver_SetPreparedPwm(MotorId_t motor, uint16_t pwm)
{
  ...
  Motor_SetCompare(motor, Motor_LimitPwm(pwm));
  return 1U;
}
                                   ...
                                   MotorDriver_Diagnostics_t *diagnostics)
{
  ...
  if ((Motor_IsValid(motor) == 0U) || (diagnostics == 0))
  {
    return 0U;
  }

  direction = motor_direction[motor];
  diagnostics->timer_number = ((motor == MOTOR_ID_A) || (motor == MOTOR_ID_B)) ?
                              8U : 5U;
  switch (motor)
  {
    case MOTOR_ID_A: diagnostics->timer_channel = 1U; break;
    case MOTOR_ID_B:
    case MOTOR_ID_C: diagnostics->timer_channel = 2U; break;
    case MOTOR_ID_D: diagnostics->timer_channel = 3U; break;
    default: return 0U;
  }
  diagnostics->pwm_arr = (uint16_t)__HAL_TIM_GET_AUTORELOAD(motor_config[motor].timer);
  diagnostics->pwm_compare = (uint16_t)__HAL_TIM_GET_COMPARE(motor_config[motor].timer,
                                                               motor_config[motor].channel);
  diagnostics->in1_expected_high = (direction == MOTOR_DIRECTION_FORWARD) ? 1U : 0U;
  diagnostics->in2_expected_high = (direction == MOTOR_DIRECTION_REVERSE) ? 1U : 0U;
  diagnostics->in1_actual_high =
    (HAL_GPIO_ReadPin(motor_config[motor].in1_port, motor_config[motor].in1_pin) ==
     GPIO_PIN_SET) ? 1U : 0U;
  diagnostics->in2_actual_high =
    (HAL_GPIO_ReadPin(motor_config[motor].in2_port, motor_config[motor].in2_pin) ==
     GPIO_PIN_SET) ? 1U : 0U;
  diagnostics->stby_high =
    (HAL_GPIO_ReadPin(MOTOR_STBY_GPIO_Port, MOTOR_STBY_Pin) == GPIO_PIN_SET) ? 1U : 0U;
  diagnostics->pwm_started = motor_pwm_started[motor];
