...
static int32_t wheel_logical_pwm[CHASSIS_WHEEL_COUNT];
...
void Chassis_Forward(uint16_t pwm)
{
  ...
  for (wheel = CHASSIS_WHEEL_FRONT_LEFT; wheel < CHASSIS_WHEEL_COUNT; wheel++)
  {
    Chassis_SetWheelLogicalPwm(wheel, (int32_t)pwm);
  }
...
void Chassis_SetWheelLogicalPwm(ChassisWheelId_t wheel, int32_t logical_pwm)
{
  ...
  uint16_t pwm;
  const NewFrontDriveMap_t *map;
  MotorDirection_t electrical_direction;

  map = NewFrontDriveMap_Get(wheel);
  if ((map == NULL) || (MotorDriver_IsEnabled() == 0U))
  {
    return;
  }
  ...
  if (logical_pwm == 0)
  {
    Motor_Stop(map->motor);
    wheel_logical_pwm[wheel] = 0;
    return;
  }

  if (((wheel_logical_pwm[wheel] > 0) && (logical_pwm < 0)) ||
      ((wheel_logical_pwm[wheel] < 0) && (logical_pwm > 0)))
  {
    /* A direction change must be separated by an explicit stopped state. */
    Motor_Stop(map->motor);
    wheel_logical_pwm[wheel] = 0;
    chassis_motion = CHASSIS_MOTION_STOP;
    chassis_pwm = 0U;
  ...
  absolute_pwm = (logical_pwm < 0) ? -(int64_t)logical_pwm : (int64_t)logical_pwm;
  if (absolute_pwm > (int64_t)MOTOR_PWM_MAX)
  {
    absolute_pwm = (int64_t)MOTOR_PWM_MAX;
  }
  pwm = (uint16_t)absolute_pwm;
  electrical_direction = map->vehicle_forward_electrical_direction;
  if (logical_pwm < 0)
  {
    electrical_direction = (electrical_direction == MOTOR_DIRECTION_FORWARD) ?
                           MOTOR_DIRECTION_REVERSE : MOTOR_DIRECTION_FORWARD;
  }

  if (electrical_direction == MOTOR_DIRECTION_FORWARD)
  {
    Motor_Forward(map->motor, pwm);
  }
  else
  {
    Motor_Reverse(map->motor, pwm);
  }

  wheel_logical_pwm[wheel] = logical_pwm;
}
...
void Chassis_UpdateFeedback(uint32_t elapsed_ms)
{
  ...
  for (wheel = CHASSIS_WHEEL_FRONT_LEFT; wheel < CHASSIS_WHEEL_COUNT; wheel++)
  {
    const NewFrontDriveMap_t *map = NewFrontDriveMap_Get(wheel);
    EncoderId_t encoder;
    int8_t sign;
