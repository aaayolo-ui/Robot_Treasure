...
typedef enum
{
  MOTOR_A_DIRECTION_STOPPED = 0,
  MOTOR_A_DIRECTION_FORWARD,
  MOTOR_A_DIRECTION_REVERSE
} MotorA_Direction_t;

static MotorA_Direction_t motor_a_direction;
...
void MotorA_Stop(void)
{
  ...
  motor_a_direction = MOTOR_A_DIRECTION_STOPPED;
}

void MotorA_Forward(uint16_t pwm)
{
  MotorA_SetForwardPwm(pwm);
}

void MotorA_SetForwardPwm(uint16_t pwm)
{
  pwm = MotorA_LimitPwm(pwm);
  ...
  if (motor_a_direction != MOTOR_A_DIRECTION_FORWARD)
  {
    MotorA_Stop();
    ...
    motor_a_direction = MOTOR_A_DIRECTION_FORWARD;
  }
...
void MotorA_Reverse(uint16_t pwm)
{
  ...
  motor_a_direction = MOTOR_A_DIRECTION_REVERSE;
}
