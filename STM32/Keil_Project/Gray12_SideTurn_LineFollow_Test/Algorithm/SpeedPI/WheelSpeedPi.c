...
static int64_t WheelSpeedPi_LimitIntegral(int64_t integral, int32_t ki_x100)
{
  int64_t limit;
...
void WheelSpeedPi_Init(WheelSpeedPi_t *controller)
{
  ...
  controller->kp_x100 = 0;
  controller->ki_x100 = 0;
                           ...
                           int32_t ki_x100)
{
  ...
  controller->kp_x100 = (kp_x100 > 0) ? kp_x100 : 0;
  controller->ki_x100 = (ki_x100 > 0) ? ki_x100 : 0;
                            ...
                            uint32_t elapsed_ms)
{
  ...
  output = ((target_magnitude * 276LL + 500LL) / 1000LL) + 14LL +
           ((control_error * (int64_t)controller->kp_x100) / 1000LL) +
           (((int64_t)controller->integral * (int64_t)controller->ki_x100) /
            1000000LL);
  ...
  /* A configured startup floor must never override the PI safety ceiling. */
  if (pwm_magnitude > (int64_t)WHEEL_SPEED_PI_PWM_MAX)
  {
    pwm_magnitude = (int64_t)WHEEL_SPEED_PI_PWM_MAX;
    controller->saturated = 1U;
  }
