...
void Chassis_SetWheelLogicalPwm(ChassisWheelId_t wheel, int32_t logical_pwm)
{
  ...
  uint16_t pwm;
  uint8_t forward_electrical_direction;

  if (((uint32_t)wheel >= (uint32_t)CHASSIS_WHEEL_COUNT) ||
      (MotorDriver_IsEnabled() == 0U))
  {
    return;
  }
  ...
  if (logical_pwm == 0)
  {
    Motor_Stop(wheel_config[wheel].motor);
    wheel_logical_pwm[wheel] = 0;
    return;
  }
  ...
  forward_electrical_direction = ((uint32_t)wheel <=
                                  (uint32_t)CHASSIS_WHEEL_FRONT_RIGHT) ? 1U : 0U;

  if ((logical_pwm > 0) == (forward_electrical_direction != 0U))
  {
    Motor_Forward(wheel_config[wheel].motor, pwm);
  }
  else
  {
    Motor_Reverse(wheel_config[wheel].motor, pwm);
  }
